...
define(["teoria", "Tone/source/MultiPlayer"], function (teoria, MultiPlayer) {
  ...
  Sampler.prototype.setBuffer = function (buffer, duration, onset) {
    var tmpBuffer = this._player.context.createBuffer(
      1,
      this._player.context.sampleRate * buffer.duration,
      this._player.context.sampleRate
    );
  ...
  Sampler.prototype.triggerAttackRelease = function (
    note,
    duration,
    time,
    velocity
  ) {
    var semitones = teoria.Interval.between(
      teoria.note("C3"),
      teoria.note(note)
    ).semitones();
