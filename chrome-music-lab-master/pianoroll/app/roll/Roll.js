...
define([
  ...
  Roll.prototype._computeStartTime = function () {
    ...
    this._computedStartTime =
      Transport.now() -
      (this._currentScroll - width / 2) / this._score.pixelsPerSecond;
  };
  ...
  Roll.prototype._onScreenNotes = function () {
    ...
    var notes = this._score.showOnScreenNotes(
      this._currentScroll - width,
      this._currentScroll
    );
    var triggerLineNotes = this._score.getTriggerLine(
      this._currentScroll - width / 2 - 1
    );
    if (triggerLineNotes) {
