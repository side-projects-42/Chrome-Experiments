define(["Tone/core/Tone", "Tone/core/Master"], function (Tone) {
   ...
   *  @private
   */
  Tone.Recorder.prototype._recordChannels = function (
    inputBuffer,
    from,
    to,
    bufferSize
    ...
    for (
      var channelNum = 0;
      channelNum < inputBuffer.numberOfChannels;
      channelNum++
    ) {
  ...
  Tone.Recorder.prototype.getAudioBuffer = function () {
    if (this.isEmpty()) {
      ...
      var audioBuffer = this.context.createBuffer(
        this.channels,
        this._recordBuffers[0].length,
        this.context.sampleRate
      );
      for (
        var channelNum = 0;
        channelNum < audioBuffer.numberOfChannels;
        channelNum++
      ) {
