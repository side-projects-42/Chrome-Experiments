  ...
  describe("Tone", function () {
    ...
    context("defaultArg", function () {
      ...
      it("handles default arguments on a shallow object", function () {
        expect(tone.defaultArg({ b: 10 }, { a: 4, b: 10 })).has.property(
          "a",
          4
        );
        expect(tone.defaultArg({ b: 10, c: 20 }, { a: 4, b: 10 })).has.property(
          "b",
          10
        );
        expect(tone.defaultArg({ b: 10, c: 20 }, { a: 4, b: 10 })).has.property(
      ...
      it("handles default arguments on a deep object", function () {
        expect(
          tone.defaultArg({ b: { c: 10 } }, { b: { c: 20, d: 30 } })
        ).has.deep.property("b.d", 30);
        expect(tone.defaultArg({ a: 10 }, { b: { c: 20 } })).has.deep.property(
          "b.c",
          20
        );
      });
    ...
    context("optionsObject", function () {
      ...
      it("gets default arguments after creating options object", function () {
        expect(
          tone.optionsObject([1, 2], ["a", "b", "c"], { c: 3 })
        ).is.deep.equal({
          a: 1,
          b: 2,
          c: 3,
      ...
      it("does not map parameter if first argument is already an object", function () {
        expect(
          tone.optionsObject([{ a: 2, b: 3 }], ["a", "b", "c"])
        ).is.deep.equal({
          a: 2,
          b: 3,
        });
    ...
    context("Tone.prototype.set / get", function () {
      ...
      it("can 'get' a value with dot notation", function () {
        ...
        //get multiple values
        expect(
          synth.get(["oscillator.type", "oscillator.phase"])
        ).to.deep.equal({
          oscillator: {
            type: "sine10",
            phase: 20,
