define([
  ...
  "Tone/core/Transport",
  "deps/teoria",
], function (Test, Tone, Transport, teoria) {
  describe("Types", function () {
    ...
    context("Frequency Conversions", function () {
      it("can convert notes into frequencies", function () {
        expect(tone.noteToFrequency("C4")).to.be.closeTo(
          teoria.note("C4").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("D4")).to.be.closeTo(
          teoria.note("D4").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("Db4")).to.be.closeTo(
          teoria.note("Db4").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("E4")).to.be.closeTo(
          teoria.note("E4").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("F2")).to.be.closeTo(
          teoria.note("F2").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("Gb-1")).to.be.closeTo(
          teoria.note("Gb-1").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("A#10")).to.be.closeTo(
          teoria.note("A#10").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("Bb2")).to.be.closeTo(
          teoria.note("Bb2").fq(),
          0.0001
        );
      });
      ...
      it("can accomidate different concert tuning", function () {
        Tone.A4 = 444;
        expect(tone.noteToFrequency("C4")).to.be.closeTo(
          teoria.note("C4").fq(Tone.A4),
          0.0001
        );
        expect(tone.noteToFrequency("D1")).to.be.closeTo(
          teoria.note("D1").fq(Tone.A4),
          0.0001
        );
        Tone.A4 = 100;
        expect(tone.noteToFrequency("C4")).to.be.closeTo(
      ...
      it("handles double accidentals", function () {
        expect(tone.noteToFrequency("Cbb4")).to.be.closeTo(
          teoria.note("Cbb4").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("Dx4")).to.be.closeTo(
          teoria.note("Dx4").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("Dbb4")).to.be.closeTo(
          teoria.note("Dbb4").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("Ex4")).to.be.closeTo(
          teoria.note("Ex4").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("Fx2")).to.be.closeTo(
          teoria.note("Fx2").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("Gbb-1")).to.be.closeTo(
          teoria.note("Gbb-1").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("Ax10")).to.be.closeTo(
          teoria.note("Ax10").fq(),
          0.0001
        );
        expect(tone.noteToFrequency("Bbb2")).to.be.closeTo(
          teoria.note("Bbb2").fq(),
          0.0001
        );
      });
      ...
      it("can convert frequencies into notes", function () {
        expect(tone.frequencyToNote(261.625)).to.equal(
          teoria.Note.fromFrequency(261.625).note.scientific()
        );
        expect(tone.frequencyToNote(440)).to.equal(
          teoria.Note.fromFrequency(440).note.scientific()
        );
        expect(tone.frequencyToNote(220)).to.equal(
          teoria.Note.fromFrequency(220).note.scientific()
        );
        expect(tone.frequencyToNote(13.75)).to.equal(
          teoria.Note.fromFrequency(13.75).note.scientific()
        );
        expect(tone.frequencyToNote(4979)).to.equal("D#8");
      });

      it("can convert note to midi values", function () {
        expect(tone.midiToNote(60)).to.equal(
          teoria.Note.fromMIDI(60).scientific()
        );
        expect(tone.midiToNote(62)).to.equal(
          teoria.Note.fromMIDI(62).scientific()
        );
      });
      ...
      it("can convert midi values to frequencie", function () {
        expect(tone.midiToFrequency(69)).to.be.closeTo(
          teoria.Note.fromMIDI(69).fq(),
          0.0001
        );
        expect(tone.midiToFrequency(57)).to.be.closeTo(
          teoria.Note.fromMIDI(57).fq(),
          0.0001
        );
        expect(tone.midiToFrequency(120)).to.be.closeTo(
          teoria.Note.fromMIDI(120).fq(),
          0.0001
        );
      });
    ...
    context("Tone.toSeconds", function () {
      ...
      it("can quantize values", function () {
        ...
        expect(tone.toSeconds("(0.4 + 4n) @ (4n + 1)")).to.be.closeTo(
          1.5,
          0.01
        );
        expect(tone.toSeconds("(0.4 @ 4n) + (2.1 @ 2n)")).to.be.closeTo(
          3.5,
          0.01
        );
      });
      ...
      it("can get the next subdivison when the transport is started", function (done) {
        ...
        setTimeout(function () {
          ...
          expect(tone.toSeconds("(+ 0.4 + 0.7 @4n) + 2n")).to.be.closeTo(
            now + 2.5,
            0.01
          );
          done();
    ...
    context("Tone.ticksToSeconds", function () {
      it("converts ticks to seconds", function () {
        ...
        expect(tone.ticksToSeconds("100i")).to.be.closeTo(
          (100 * (60 / bpm)) / ppq,
          0.01
        );
      });
