...
Thread.prototype.updGrab = function () {
  ...
  if (this.fishGrab != null) {
    var xu = this.fishGrab.xp1;
    var yu = this.fishGrab.yp1;
    // else grabbed by user
  } else {
    var xu = this.m.getMouseX();
    var yu = this.m.getMouseY();
  }
...
Thread.prototype.checkInstantGrab = function () {
  ...
  if (
    Math.abs(this.distPerp) < this.distInstantPerp &&
    this.rGrab > 0 &&
    this.rGrab < 1
  ) {
...
Thread.prototype.setPosition = function (x0, y0, x1, y1) {
  ...
  // set my pitch
  if (this.len > this.len1) {
    this.rPitch = 0;
  } else if (this.len < this.len0) {
    this.rPitch = 1;
  } else {
    this.rPitch = 1 - norm(this.len, this.len0, this.len1);
  }
