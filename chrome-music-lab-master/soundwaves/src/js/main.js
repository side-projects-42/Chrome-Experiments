...
if (gl) {
  ...
  let resize = () => {
    ...
    if (keys)
      [].forEach.call(
        keys,
        (el) => (el.style.height = window.innerHeight * (1.0 - keyRatio) + "px")
      );
  ...
  let getViewport = () => {
    ...
    let view = [
      ...
      window.innerHeight * pixelRatio * keyRatio,
    ];
    size = Math.max(...view);
  ...
  function renderStep(t) {
    ...
    amplitude = math.map(
      synth.envelope._sig.getValueAtTime(synth.toSeconds()),
      0,
      0.3,
      0,
      1 / 25
    ...
    pOffset += ((isFocused ? 0.5 : 0.0) - pOffset) * 0.1;
    background(
      gl,
      time,
      delta,
    ...
    let displacement =
      Math.sin(time * 1.5) *
      10.0 *
      math.normalize(synth.envelope.value, 0, 0.3);
