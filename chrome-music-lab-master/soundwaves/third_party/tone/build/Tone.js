(function (root) {
  ...
  Module(function (Tone) {
    ...
    Object.defineProperty(Tone.Envelope.prototype, "attackCurve", {
      ...
      set: function (type) {
        if (
          type === Tone.Envelope.Type.Linear ||
          type === Tone.Envelope.Type.Exponential
        ) {
          this._attackCurve = type;
        } else {
          throw Error(
            'attackCurve must be either "linear" or "exponential". Invalid type: ',
            type
          );
        }
      },
    ...
    Object.defineProperty(Tone.Envelope.prototype, "releaseCurve", {
      ...
      set: function (type) {
        if (
          ...
          throw Error(
            'releaseCurve must be either "linear" or "exponential". Invalid type: ',
            type
          );
        }
      },
    ...
    Tone.Envelope.prototype.triggerAttack = function (time, velocity) {
      ...
      this._sig.exponentialRampToValueAtTime(
    ...
    Tone.Envelope.prototype.triggerRelease = function (time) {
      ...
      if (this._releaseCurve === Tone.Envelope.Type.Linear) {
        this._sig.linearRampToValueBetween(
          this._minOutput,
          time,
          time + release
        );
      } else {
        this._sig.exponentialRampToValueBetween(
          this._minOutput,
          time,
          release + time
        );
    ...
    Tone.Expr.prototype._parseTree = function (expr) {
      ...
      function parseExpression(precedence) {
        ...
        while (matchGroup(token, "binary", precedence)) {
          ...
          expr = {
            ...
            method: token.method,
            args: [expr, parseExpression(precedence)],
          };
          token = lexer.peek();
        }
        return expr;
      ...
      function parsePrimary() {
        ...
        throw new SyntaxError(
          "Parse error, cannot process token " + token.value
        );
      }
      function parseFunctionCall(func) {
        ...
        if (!matchSyntax(token, "(")) {
          throw new SyntaxError(
            'Expected ( in a function call "' + func.value + '"'
          );
        }
        token = lexer.peek();
        if (!matchSyntax(token, ")")) {
  ...
  Module(function (Tone) {
    ...
    Tone.Transport = function () {
      ...
      //	SWING
      //////////////////////////////////////////////////////////////////////
      var swingSeconds = this.notationToSeconds(
        TransportConstructor.defaults.swingSubdivision,
        TransportConstructor.defaults.bpm,
       ...
       */
      this._swingTicks =
        (swingSeconds / (60 / TransportConstructor.defaults.bpm)) * this._ppq;
      /**
       *  The swing amount
       *  @type {NormalRange}
       *  @private
    ...
    Tone.Transport.prototype.setInterval = function (callback, interval) {
      console.warn(
        "This method is deprecated. Use Tone.Transport.scheduleRepeat instead."
      );
    ...
    Tone.Transport.prototype.clearInterval = function (id) {
      console.warn(
        "This method is deprecated. Use Tone.Transport.clear instead."
      );
    ...
    Tone.Transport.prototype.setTimeout = function (callback, timeout) {
      console.warn(
        "This method is deprecated. Use Tone.Transport.scheduleOnce instead."
      );
    ...
    Tone.Transport.prototype.setTimeline = function (callback, time) {
      console.warn(
        "This method is deprecated. Use Tone.Transport.schedule instead."
      );
  ...
  Module(function (Tone) {
    ...
    Tone.Meter = function () {
       ...
       */
      this._jsNode = this.context.createScriptProcessor(
        options.bufferSize,
        this._channels,
        1
      );
    ...
    Tone.Meter.prototype._onprocess = function (event) {
      ...
      for (var channel = 0; channel < this._channels; channel++) {
        ...
        this._volume[channel] = Math.max(
          rms,
          this._volume[channel] * smoothing
        );
  ...
  Module(function (Tone) {
    ...
    Object.defineProperty(Tone.Buffer, "onload", {
      set: function (cb) {
        console.warn(
          "Tone.Buffer.onload is deprecated, use Tone.Buffer.on('load', callback)"
        );
    ...
    Object.defineProperty(Tone.Buffer, "onprogress", {
      set: function (cb) {
        console.warn(
          "Tone.Buffer.onprogress is deprecated, use Tone.Buffer.on('progress', callback)"
        );
    ...
    Object.defineProperty(Tone.Buffer, "onerror", {
      set: function (cb) {
        console.warn(
          "Tone.Buffer.onerror is deprecated, use Tone.Buffer.on('error', callback)"
        );
  ...
  Module(function (Tone) {
    ...
    Tone.Note = function (channel, time, value) {
       ...
       *  @private
       */
      this._timelineID = Tone.Transport.setTimeline(
        this._trigger.bind(this),
        time
      );
    };
    /**
  ...
  Module(function (Tone) {
          ...
          if (event.state === Tone.State.Started) {
            ...
            if (this._loop) {
              ...
              event.id = Tone.Transport.scheduleRepeat(
                ...
                this._getLoopDuration().toString() + "i",
                startTick + "i",
                duration
              );
            } else {
    ...
    Tone.Event.prototype._tick = function (time) {
      if (
        !this.mute &&
        this._state.getStateAtTime(Tone.Transport.ticks) === Tone.State.Started
      ) {
  ...
  Module(function (Tone) {
    ...
    Object.defineProperty(Tone.OmniOscillator.prototype, "type", {
      ...
      set: function (type) {
        if (
          type.indexOf("sine") === 0 ||
          type.indexOf("square") === 0 ||
          type.indexOf("triangle") === 0 ||
          type.indexOf("sawtooth") === 0 ||
          type === Tone.Oscillator.Type.Custom
        ) {
  ...
  Module(function (Tone) {
    ...
    Tone.Sampler.prototype._loadBuffers = function (urls) {
      if (this.isString(urls)) {
        this._buffers["0"] = new Tone.Buffer(
          urls,
          function () {
            this.sample = "0";
          }.bind(this)
  ...
  Module(function (Tone) {
     ...
     *  @private
     */
    Tone.SimpleSynth.prototype._triggerEnvelopeAttack = function (
      time,
      velocity
    ) {
      //the envelopes
  ...
  Module(function (Tone) {
    ...
    Tone.ExternalInput.prototype._getUserMedia = function (callback) {
      ...
      if (Tone.ExternalInput.sources[this._inputNum]) {
        this._constraints = {
          audio: {
            optional: [
              { sourceId: Tone.ExternalInput.sources[this._inputNum].id },
            ],
          },
        };
      }
    ...
    Tone.ExternalInput.getSources = function (callback) {
      if (
        Tone.ExternalInput.sources.length === 0 &&
        Tone.ExternalInput._canGetSources
      ) {
