...
function rebuildFBO(fbo) {
  ...
  if (numColors === 0) {
    fbo._color_rb = initRenderBuffer(
      gl,
      width,
      height,
      gl.RGBA4,
  ...
  if (WEBGL_depth_texture) {
    ...
    if (useDepth && useStencil) {
      fbo._depth_rb = initRenderBuffer(
        gl,
        width,
        height,
        gl.DEPTH_STENCIL,
  ...
  if ("color" in options) {
    ...
    if (numColors > 1) {
      ...
      if (!WEBGL_draw_buffers) {
        ...
        numColors >
        gl.getParameter(WEBGL_draw_buffers.MAX_COLOR_ATTACHMENTS_WEBGL)
      ) {
        throw new Error(
          "gl-fbo: Context does not support " + numColors + " draw buffers"
        );
      }
    }
  }
