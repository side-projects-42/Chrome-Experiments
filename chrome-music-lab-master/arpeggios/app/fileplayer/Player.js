...
define([
  "Tone/source/MultiPlayer",
  "./Notes",
  "Tone/core/Master",
  "Tone/core/Buffer",
], function (MultiPlayer, Notes, Master, Buffer) {
  /**
   * A sample player
  ...
  Player.prototype.load = function () {
    ...
    for (var i = 0; i < allNotes.length; i += this._stepSize * 2 + 1) {
      ...
      if (typeof bufferPitch !== "undefined") {
        ...
        this._loadCount += 1;
        var buff = new Buffer(
          this._instrumentFolder + "/" + bufferPitch + ".mp3",
          this._loadedBuffer.bind(this)
        );
  ...
  Player.prototype.triggerAttackRelease = function (note, duration, time) {
    ...
    this._multiPlayer.start(description.buffer, time, {
      playbackRate: this._multiPlayer.intervalToFrequencyRatio(
