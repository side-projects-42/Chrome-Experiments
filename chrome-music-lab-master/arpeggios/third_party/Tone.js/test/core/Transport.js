define(["Test", "Tone/core/Transport", "Tone/core/Tone"], function (
  Test,
  Transport,
  Tone
) {
  describe("Transport", function () {
    ...
    context("BPM and timeSignature", function () {
      ...
      it("can get and set bpm", function () {
        ...
        //reset the bpm
        Tone.Transport.bpm.value = 120;
        expect(Tone.Transport.bpm._param.value).to.equal(
          2 * Tone.Transport.PPQ
        );
      });
    ...
    context("looping", function () {
      ...
      it("can get and set loop points", function () {
        ...
        expect(Tone.Transport.loopEnd).to.be.closeTo(
    ...
    context("Quantization", function () {
      ...
      it("returns the time quantized to the next subdivision", function () {
        ...
        expect(Tone.Transport.quantize("1m", "2m")).to.be.closeTo(
          Tone.Transport.toSeconds("2m"),
          0.01
        );
        expect(Tone.Transport.quantize(2.3, 0.5)).to.be.closeTo(2.5, 0.01);
        expect(Tone.Transport.quantize("4n", "8n")).to.be.closeTo(
          Tone.Transport.toSeconds("4n"),
          0.01
        );
      ...
      it("returns now relative times with the Transport stopped", function () {
        ...
        expect(Tone.Transport.quantize(undefined, "1m")).to.be.closeTo(
          now,
          0.01
        );
        expect(Tone.Transport.quantize("+1m", "1m")).to.be.closeTo(
          now + Tone.Transport.toSeconds("1m"),
          0.01
        );
      });
      ...
      it("returns the time of the next subdivision when the transport is started", function (done) {
        ...
        setTimeout(function () {
          ...
          expect(Tone.Transport.quantize(undefined, 0.5)).to.be.closeTo(
            now + 0.5,
            0.01
          );
          expect(Tone.Transport.quantize("+1.1", 0.5)).to.be.closeTo(
            now + 1.5,
            0.01
          );
          expect(Tone.Transport.quantize("+0.4", 1)).to.be.closeTo(
            now + 1,
            0.01
          );
          expect(Tone.Transport.quantize("+1.1", 1)).to.be.closeTo(
            now + 2,
            0.01
          );
          done();
    ...
    context("scheduleRepeat", function () {
      ...
      it("scheduled event gets invoked with the time of the event", function (done) {
        var startTime = Tone.Transport.now() + 0.1;
        var eventID = Tone.Transport.scheduleRepeat(
          function (time) {
      ...
      it("can schedule multiple events and invoke them in the right order", function (done) {
        var first = false;
        var firstID = Tone.Transport.scheduleRepeat(
          function () {
            first = true;
        ...
        var secondID = Tone.Transport.scheduleRepeat(
          function () {
