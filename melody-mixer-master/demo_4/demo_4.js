...
function interpolateMelodies() {
  interpolatedNoteSequences = musicVAE.interpolate(
    [MELODY1, MELODY2],
    numInterpolations
  );
...
function draw() {
  //playback logic
  var totalPlayTime =
    (Tone.Transport.bpm.value * NUM_STEPS * numInterpolations) / 1000;
  percent = (Tone.Transport.seconds / totalPlayTime) % 1;
  if (Tone.Transport.state === "started") {
    var currSequenceIndex = Math.floor(percent * numInterpolations);
    var currStepIndex = Math.floor(
      (percent * numInterpolations - currSequenceIndex) * NUM_STEPS
    );
    if (currStepIndex != stepIndex) {
      var notes = interpolatedNoteSequences[currSequenceIndex].notes.filter(
        function (note) {
  ...
  if (mousePressedInScrubBar()) {
    ...
    if (mouseX > tileLeft(0) && mouseX < tileRight(numInterpolations - 1)) {
      seekTime =
        ((mouseX - tileLeft(0)) / calcCurrentSequenceWidth()) * totalPlayTime;
  ...
  drawPlayhead(
    new p5.Vector(
      tileLeft(0) + percent * calcCurrentSequenceWidth(),
      SCRUB_BAR_HEIGHT
    ),
    SCRUB_BAR_GAP
  );
...
function drawPlayhead(pos, triangleHeight) {
  ...
  triangle(
    ...
    pos.y,
    pos.x,
    pos.y + triangleHeight,
    pos.x + hWidth,
    pos.y
  );
  stroke(255);
  strokeWeight(2);
  line(pos.x, pos.y + 2, pos.x, sequences.tileSize + pos.y + triangleHeight);
}
...
function drawSequences() {
  ...
  if (interpolatedNoteSequences) {
    if (
      mousePressedInSequenceTiles() ||
      sequences.width !== sequences.targetWidth
    ) {
      drawNotes(
        interpolatedNoteSequences[0].notes,
        tileLeft(0),
        sequences.tileSize
      );
      drawNotes(
        interpolatedNoteSequences[interpolatedNoteSequences.length - 1].notes,
        tileLeft(completedTiles - 1) + partSize,
        sequences.tileSize
      );
    } else {
      for (var i = 0; i < interpolatedNoteSequences.length; i++) {
        if (interpolatedNoteSequences.length > i) {
          drawNotes(
            interpolatedNoteSequences[i].notes,
            tileLeft(i),
            sequences.tileSize
          );
        }
...
function drawNotes(notes, x, size) {
  ...
  notes.forEach(function (note) {
    ...
    rect(
      ...
      cellWidth * (note.quantizedEndStep - note.quantizedStartStep) -
        emptyNoteSpacer,
      cellHeight
    );
  });
...
function tileLeft(index) {
  return (
    width / 2 - calcCurrentSequenceWidth() / 2 + sequences.tileSize * index
  );
}
...
function mousePressedInSequenceTiles() {
  return (
    mouseDownPosition &&
    mouseDownPosition.y <
      height / 2 + SCRUB_BAR_HEIGHT + sequences.tileSize / 2 &&
...
function calcCurrentSequenceWidth() {
  ...
  var isMovingTowardsCenter =
    abs(mouseX - width / 2) < abs(mouseDownPosition.x - width / 2);
  ...
  return Math.min(
    Math.max(currWidth, sequences.tileSize * 2),
    width - (width % sequences.tileSize)
  );
}
