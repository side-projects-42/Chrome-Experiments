...
new musicvae.MusicVAE(melodiesModelCheckPoint)
  ...
  .then(function (noteSequences) {
    var text =
      "Click to Play a blend from Melody 1 to Melody 2 in " +
      numInterpolations +
      " interpolations";
...
function draw() {
  ...
  if (Tone.Transport.state === "started") {
    ...
    if (currStepIndex != stepIndex) {
      ...
      var notes =
        interpolatedNoteSequences[currSequenceIndex].notes.filter(
          isCurrentStep
        );
...
function drawNotes(notes, x, y, width, height) {
  ...
  notes.forEach(function (note) {
    ...
    rect(
      ...
      height - cellHeight * (note.pitch - MIDI_START_NOTE),
