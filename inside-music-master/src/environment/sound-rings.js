...
window.AFRAME.registerComponent("sound-rings", {
  ...
  init() {
    ...
    this.waveformData = take(
      this.numRings,
      (i) => new Float32Array(this.pointsPerRing)
    );
  ...
  __cacheProperties() {
    ...
    this.__originalOpacities = this.rings.map(
      (r) => r.material.uniforms.opacity.value
    );
    this.__originalRadii = this.rings.map(
      (r) => r.material.uniforms.radius.value
    );
  },
    ...
    this.loop
      ...
      .on("update", (delta, elapsed) => {
        this.rings.forEach((r, i, arr) => {
          ...
          r.material.uniforms.radius.value = map(
            t,
            0,
            1,
            isActive ? 0 : startRad,
          ...
          r.material.uniforms.opacity.value = map(
            t,
            0,
            1,
            isActive ? 0 : startOp,
  ...
  update() {
    ...
    const texture = new THREE.TextureLoader().load(
      `images/textures/${this.data.shape}.png`
    );

    this.rings.forEach((ring, i, arr) => {
      ring.material.uniforms.size.value =
        this.data.size * (window.devicePixelRatio || 1);
      ring.material.uniforms.color.value = this.startColor
        .clone()
        .lerp(this.endColor, i / arr.length);
  ...
  tick() {
    ...
    this.amplitude = Math.max(
      nextAmp,
      this.amplitude + (nextAmp - this.amplitude) * 0.1
    );
