...
AFRAME.registerComponent("sphere", {
  ...
  init() {
    ...
    sphereEl.setAttribute(
      "material-sphere",
      "opacity: 0.8; tintColor: #0000ff"
    ); //`transparent : true; color: ${this._inactiveColor}; opacity: 0.8; shader : flat`)
  ...
  tick() {
    ...
    this.sphereEl.components[
      "material-sphere"
    ].material.uniforms.mixRate.value = smoothAmp * 2.5;
  },

  _addText(sphereEl) {
    const textEntity = (this.text = document.createElementWithAttributes(
      "a-text",
      {
        value: "",
        position: "0 2 0",
    ...
    const fadeInAnimation = document.createElementWithAttributes(
      "a-animation",
      {
        attribute: "opacity",
        begin: "floor-on",
    ...
    const fadeOutAnimation = document.createElementWithAttributes(
      "a-animation",
      {
        attribute: "opacity",
        begin: "floor-off",
  ...
  _addEvents(sphereEl) {
    ...
    this.el.addEventListener("componentchanged", (e) => {
      if (e.detail.name === "active") {
        ...
        sphereEl.components["material-sphere"].material.uniforms.enabled.value =
          e.detail.newData ? 1 : 0;
      }
    });
  ...
  update() {
    ...
    this.sphereEl.setAttribute(
      "material-sphere",
      `opacity: 0.8; tintColor: ${this.data.tintColor}`
    );
  },
});
