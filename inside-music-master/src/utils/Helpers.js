 ...
 * @returns {boolean}
 */
export const isMobile = () => document.querySelector("a-scene").isMobile;
...
export const isTablet = () =>
  Math.max(window.screen.width, window.screen.height) /
    ...
    1.35 && !/(Oculus|Gear)/.test(navigator.userAgent);
...
Document.prototype.createElementWithAttributes = function createElement(
  name,
  attrs
) {
  // create the element
...
export const randVector3 = (min, max) =>
  new THREE.Vector3(rand(min, max), rand(min, max), rand(min, max));
