...
AFRAME.registerComponent("retical-controller", {
  init: function () {
    ...
    setTimeout(() => {
      this.el.setAttribute(
        "ray-intersection-emitter",
        "front: [retical]; back: [camera]; touch:false; updatePositions: true;"
      );
    }, 100);
  },
...
AFRAME.registerComponent("custom-gearvr-controller", {
  ...
  init: function () {
    ...
    setTimeout(() => {
      this.el.setAttribute(
        "ray-intersection-emitter",
        "front: .tracked-ray-front-right; back: #righthand;"
      );
    }, 100);
  },
...
AFRAME.registerComponent("custom-six-dof-controller", {
  ...
  update: function () {
    if (this.data.numberOfControllers > this.controllerEntities.length) {
      for (
        let i = this.el.children.length;
        i < this.data.numberOfControllers;
        i++
      ) {
        this.createController(i);
      }
    } else {
      this.controllerEntities.forEach(
        (entity) =>
...
AFRAME.registerComponent("controls", {
  ...
  updateControls: function () {
    ...
    const changed =
      diff(
        pluck(this.ping, "type", empty(tmpA)),
        pluck(this.pong, "type", empty(tmpB))
      ) ||
      this.el.hasAttribute(components[types.RETICAL]) !==
        shouldGetRetical(this.pong);
    ...
    if (this.pong[0].type === types.VIVE) {
      this.el.setAttribute(
        components[types.VIVE],
        "numberOfControllers: " + this.pong.length
      );
      this.resetPingPong();
      return;
    }
