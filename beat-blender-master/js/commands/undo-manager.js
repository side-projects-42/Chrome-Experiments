...
export class UndoManager {
  ...
  constructor(app, key, itemKey, max = 64) {
    ...
    assert(
      Array.isArray(app.state[itemKey]),
      `${key} should be an Array in state`
    );
    assert(
      Array.isArray(this.get(0)),
      `${itemKey} should be a 2D Array in state`
    );
    assert(
      Array.isArray(app.state[itemKey][0]),
      `${itemKey} should be a 2D Array in state`
    );
    ...
    assert.equal(
      typeof app.setDeep,
      "function",
      "app should have a setDeep function"
    );
  }
