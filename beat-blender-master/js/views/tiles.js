...
const findIndexForInput = (el, depth = 0, maxDepth = 5) => {
  const foundCorners = this.cornerOrder.filter((corner) =>
    el.classList.contains(corner)
  );
...
export class Corners extends View {
  ...
  shouldComponentUpdate(state, last) {
    return anyKeyNotEqual(state, last, [
      "innerWidth",
      "innerHeight",
      "selectedCornerIndex",
    ]);
  ...
  render(state) {
    ...
    const tW =
      state.layout === MOBILE_PORTRAIT
        ? state.innerWidth
        : cw * (li[1].column - li[0].column) * state.innerWidth;
    ...
    this.domElement.style.display =
      state.selectedCornerIndex === -1 ? "none" : "block";
