...
export class CustomizeBeatsButton extends ToggleButton {
  ...
  shouldComponentUpdate(state, last) {
    return (
      state.selectedCornerIndex !== last.selectedCornerIndex ||
  ...
  render(state) {
    const isBusy =
      state.interpolating ||
      (state.gridAnimationLerp > 0 && state.gridAnimationLerp < 1);
...
export class DragDrawToggle extends View {
  ...
  render({ playMode, selectedCornerIndex }) {
    ...
    this.domElement.classList[playMode === playModes.PATH ? "add" : "remove"](
      "right-selected"
    );
    this.domElement.classList[selectedCornerIndex === -1 ? "add" : "remove"](
      "active"
    );
...
export class BPMSlider extends ToggleButton {
  ...
  render(state, last) {
    ...
    const perc =
      (state.sequence.activeColumn / state.sequence.columns) * Math.PI;
