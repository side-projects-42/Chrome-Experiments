...
export class SequencerGridView extends View {
  ...
  render(state, last) {
    if (
      ...
      //resize
      const width =
        state.layout === responsive.MOBILE_PORTRAIT
          ? state.innerWidth
          : state.innerWidth * 0.5 * 0.8;
      setCanvasSize(this.domElement, width, width / 2);
    }

    const c = this.context;
    const { sequence, encodedSequences, selectedIndex, selectedCornerIndex } =
      state;

    this.domElement.style.cursor =
      selectedCornerIndex >= 0 ? "pointer" : "inherit";

    c.clearRect(0, 0, c.canvas.width, c.canvas.height);
    const matrix = notesToMatrix(
      encodedSequences[selectedIndex],
      sequence.columns,
      sequence.rows
    );
    ...
    const colors = times(sequence.columns, (x) => {
      let opacity =
        x === sequence.activeColumn ? 1 : (x / 8) % 2 < 1 ? 0.33 : 0.2;
      if (selectedCornerIndex >= 0) {
        opacity += 0.2;
      }

      return times(sequence.rows, (y) =>
        !!matrix[x][y]
