...
const sizeEqual = (state, last) =>
  allKeysEqual(state, last, ["innerWidth", "innerHeight", "layout"]);
const gridEqual = (state, last) =>
  allKeysEqual(state, last, ["innerWidth", "innerHeight", "sequenceUUID"]);
...
export class GridView extends View {
  constructor(domElement) {
    ...
    const onMouseMove = (event, state) => {
      ...
      if (
        (event.touches && event.touches.length && isPuckHit) ||
        state.playMode === playModes.PATH
      ) {
        event.preventDefault();
      }

      this.domElement.style.cursor = hitsPuck(this.cursor, state.puck, 0.03)
        ? "pointer"
        : "inherit";

      const closestCell = grid2d.intersectsCellPosition(
        state.grid,
        this.cursor
      );
      if (!closestCell) {
  ...
  getSequencePattern(state, last) {
    ...
    const { grid, fullGradient: colors } = state;
    ...
    tmpGrid.paddingRight =
      tmpGrid.paddingLeft =
      tmpGrid.paddingTop =
      tmpGrid.paddingBottom =
        2;

    //create a new image data for all the rendered sequences
    this.__cachedSequencePattern = c.createPattern(
      renderToNewCanvas(this.context.canvas, (ctx) => {
  ...
  getGradientPattern(state, last) {
    ...
    this.__cachedGradientPattern = c.createPattern(
      renderToNewCanvas(c.canvas, (ctx) => {
  ...
  getCells(state, last) {
    ...
    this.__cells = values(cellReduce).sort((a, b) =>
      a[0].distance < b[0].distance ? 1 : -1
    );
  ...
  __shouldStartTransitionIn(
    { isShowingSplash, gridAnimationLerp, selectedCornerIndex },
    last
  ) {
  ...
  render(state, last) {
    if (state.selectedCornerIndex !== last.selectedCornerIndex) {
      ...
      this.domElement.style.pointerEvents =
        state.selectedCornerIndex === -1 ? "all" : "none";
    }
    ...
    let { grid, gridAnimationLerp: t } = state;
      ...
      cells.forEach((nested, i, arr) => {
        ...
        const alph =
          i === 0
            ? maxAlpha
            : clamp(
                scalemap(t, (1 / (arr.length - 1)) * i, 0.9, 0, maxAlpha),
                0,
                maxAlpha
              );
        ...
        nested.forEach((cell) =>
          renderCell(c, cell, alph, gradientPattern, seqPattern, width, height)
        );
      });
    } else {
