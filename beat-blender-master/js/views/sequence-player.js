...
export class SequencePlayer extends View {
  ...
  render(state, last) {
    ...
    if (!this.player) {
      assert(
        Array.isArray(sampleURLs),
        `sampleURLs should be an array, received ${sampleURLs}`
      );
    ...
    const channel = 10;
    const hasNotesOn =
      (this.notesOn.length > 0 && notes.length > 0) ||
      state.sequence.activeColumn == 1;
    ...
    for (let i = 0; i < notes.length; i++) {
      ...
      if (midiDeviceName && state.selectedMidiOutIndex >= 0) {
        ...
        const time =
          state.sequence.time !== last.sequence.time
            ? state.sequence.time
            : Tone.Transport.now();
