...
import {
  generate4PointGradientAt,
  generate4PointGradient,
} from "./models/color";
import {
  notesToMatrix,
  encodedNotesToINoteSequence,
  iNoteSequenceToEncodedNotes,
  getLabelsFromEncoding,
} from "./samples";
...
const gtag =
  window.gtag ||
  function () {
    console.warn("gtag not found, did not track", arguments);
  };
const gEvent = function () {
  gtag.apply(null, ["event", ...arguments]);
};
  ...
  computed: {
    cornerIndices: (state, last) => {
      ...
      if (
        grid &&
        last.grid &&
        grid.columns === last.grid.columns &&
        grid.rows === last.grid.rows &&
        state.cornerIndices
      ) {
    ...
    "sequence.colorOn": (state, last) => {
      const { grid, selectedIndex, gradient } = state;
    ...
    layoutItems: (state, last) => {
      ...
      const aspectRatio = (a, b) =>
        (((b.column - a.column) / g.columns) * state.innerWidth) /
        (((b.row - a.row) / g.rows) * state.innerHeight);
...
let musicVAESampler = new MusicVAE(
  "https://storage.googleapis.com/download.magenta.tensorflow.org/models/music_vae/dljs/drums_lokl_q8"
);
let musicVAE = new MusicVAE(
...
const updateMusicVAE = () => {
  ...
  const corners = [0, 2, 1, 3]
    .map((i) => cornerIndices[i])
    .map((index) => encodedSequences[index])
    .map((sequence) =>
      encodedNotesToINoteSequence(sequence, app.state.sequence.columns)
    );
  const output = musicVAE.interpolate(corners, grid.columns);

  const encodedNoteSequences = iNoteSequenceToEncodedNotes(
    output,
    app.state.sequence.columns
  );

  [0, 2, 1, 3].forEach(
    (i, j) =>
      (encodedNoteSequences[cornerIndices[i]] = iNoteSequenceToEncodedNotes(
        [corners[j]],
        app.state.sequence.columns
      )[0])
  ); //swap out reconstructed corners with the original drums

  app.set({
    sequenceUUID: uuid(),
    encodedSequences: encodedNoteSequences,
    allSequences: output.map((noteSequence) =>
      noteSequence.constructor.toObject(noteSequence)
    ),
  });
};
...
const sampleFromLatentSpace = (numSamples) =>
  iNoteSequenceToEncodedNotes(
    musicVAESampler.sample(numSamples, app.state.sampleTemperature),
    app.state.sequence.columns
  );
...
app.addView([
  ...
  new SequencePlayer().on("note", (view, time, col) => {
  ...
  new Corners("#group-corners").on("click", (view, i) => {
  ...
  new SequencerGridView("#sequencer-view")
    ...
    .on("hover", (view, cellPosition) =>
      app.setDeep("sequence.hoverCellPosition", cellPosition)
    )
    .on("toggle", (view, cellPosition) => {
      ...
      const matrix = notesToMatrix(
        sequenceNotes,
        app.state.sequence.columns,
        app.state.sequence.rows
      );
      const shift = !matrix[cellPosition.column][cellPosition.row]
        ? note
        : -1 * note;
  ...
  new EditModeGroup(".edit-mode-group")
    ...
    .on("clear", () => {
      ...
      undoManager.pushHistory(
        selectedCornerIndex,
        cornerIndices[selectedCornerIndex]
      );
      app.setDeep(
  ...
  new DragDrawToggle("#drag-draw-toggle").on("click", () => {
    const playMode =
      app.state.playMode === playModes.DRAG ? playModes.PATH : playModes.DRAG;
    app.set({
      playMode,
      path: [],
    });
  ...
  new ToggleButton("#play-pause-button", (state) => state.playing).on(
    "click",
    () => app.set({ playing: !app.state.playing })
  ),
  new CustomizeBeatsButton(
    "#edit-corners-button",
    (state) => state.selectedCornerIndex > 0
  )
    .on("done", () => {
      gEvent("interpolate", "done-button");
      //calculate where the puck is on the grid and return it as the selectedIndex
      const selectedIndex = grid2d.intersectsCellIndex(
        app.state.grid,
        app.state.puck
      );
    ...
    .on("edit", () => {
      gEvent("edit");
      const selectedCornerIndex =
        app.state.lastSelectedCornerIndex !== -1
          ? app.state.lastSelectedCornerIndex
          : 0;
  ...
  new SelectMidiOut("#midi-out-selector").on(
    "change",
    (view, selectedMidiOutIndex) => {
      gEvent("change-midi-out");
      app.set({ selectedMidiOutIndex });
      //find out wha index was selected
      console.log(
        `current midiOut: ${app.state.midiOut[selectedMidiOutIndex]}`
      );
    }
  ),
  ...
  //simplistic view wrapping the body
  {
    ...
    render: function (state) {
      document.querySelector(".wrapper").style.display =
        app.state.modal === modals.NONE && !app.state.isShowingSplash
          ? "block"
          : "none";
...
function updatePathLerp() {
  ...
  let pathLerpDelta =
    loop.deltaTime /
    1000 /
    (Tone.Transport.toSeconds("16n") *
      state.sequence.columns *
      state.pathIntersections.length);
...
page("*", function initializeMusicVAE(ctx, next) {
  ...
  if (!musicVAEInitializing) {
    ...
    musicVAE.initialize().then(function () {
