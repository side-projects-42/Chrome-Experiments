 ...
 * @type {Array<string>}
 */
export const urls = notesBySample.map(
  (notes) => `/ai/beat-blender/view/assets/audio/drums/${notes[0]}.mp3`
);
...
export const iNoteSequenceToEncodedNotes = (sequences, columns) => {
  const encodedSequences = times(sequences.length, () =>
    times(columns, () => 0)
  );
  sequences.forEach((sequence, sequenceIndex) => {
    sequence.notes.forEach((note) => {
      encodedSequences[sequenceIndex][note.quantizedStartStep] += Math.pow(
        2,
        getDrumRowFromNotePitch(note.pitch)
      );
    });
  });
...
export const encodedNotesToINoteSequence = (notes, columns) => {
  ...
  notes.forEach((note, column) => {
    ...
    notes.forEach((row) => {
      sequenceNotes.push({
        pitch: getMidiNoteFromDrumRow(row),
        quantizedStartStep: column,
      });
      // matrix[column][row] = 1;
    });
  });
