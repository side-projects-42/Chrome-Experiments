...
export default function app({ options, state, computed }) {
  ...
  function addView(view) {
    ...
    for (let prop in view.events) {
      events[prop] = (event) =>
        view.events[prop](event, app.state, app.lastState);
    }

    if (Object.keys(events).length) {
      //every view has a uuid and that will be used for registering the event map
      removeEventsMap[id] = view.domElement
        ? eventMap(view.domElement, events)
        : eventMap(events);
    }
  ...
  function render() {
    ...
    for (let i = 0; i < views.length; i++) {
      const view = views[i];
      if (
        !view.shouldComponentUpdate ||
        view.shouldComponentUpdate(app.state, app.lastState)
      ) {
