...
var Visualizer = (module.exports = function (x) {
  ...
  this.createDraggers = function () {
    ...
    var onDragging = function (event) {
      ...
      var label;
      dragging.x =
        ((event.detail.x + scope.base.position.x) / window.innerWidth) * 2;
      dragging.y =
        ((event.detail.y + scope.base.position.y) / window.innerHeight) * 2;
      ...
      if (result[smallestId].index != scope.storedSoundIndex) {
        ...
        selectedDragger.setFocusPosition(
          scope.storedSoundIndex,
          scope.panner,
          scope.zoomer
        );
        ...
        scope.dispatchEvent("ON_DRAG_SELECT", [
          selectedDragger.draggerIndex,
          scope.storedSoundIndex,
        ]);
      }
    };
    ...
    var onPinchStarted = function (event) {
      ...
      var onPinchMoved = function (event) {
        ...
        var size =
          startScale + (touchZoomDistanceEnd - touchZoomDistanceStart) * 0.025;

        var scalarWidth = window.innerWidth / 1000;
        var scalarHeight =
          (window.innerHeight - controller.clientHeight) / 1000;
        var resetScale =
          scalarWidth < scalarHeight ? scalarWidth : scalarHeight;
    ...
    var onTouchStarted = function (event) {
      ...
      switch (event.touches.length) {
        ...
        case 2:
          ...
          scope.context.removeEventListener(
            "touchstart",
            onTouchStarted,
            false
          );
  ...
  this.createZoomElements = function () {
    ...
    this.zoomControls.addEventListener(
      "ON_ZOOM_IN_CLICKED",
      function () {
    ...
    var onWheel = function (event) {
      ...
      if (delta > 0) {
        ...
        Data.cloudSize2D =
          Data.cloudSize2D < resetScale ? resetScale : Data.cloudSize2D;
        scalar = Data.cloudSize2D;
        scope.zoomer.scale.set(scalar, scalar, scalar);
        scope.updateDraggers();
      } else {
  ...
  this.onBgDown = function (event) {
    ...
    var draggerStart = new THREE.Vector2(
      scope.panner.position.x,
      scope.panner.position.y
    );
