...
var Data = (module.exports = {
  ...
  importChunk: function (chunkId, callback) {
    ...
    var onTSNELoaded = function (response) {
      Data.loadPercentage =
        (((chunkId + 1) / (Data.totalChunks - 1)) * 100) | 0;
      ...
      for (i = 0; i < total; i++) {
        ...
        Data.myRTree.insert(
          { x: pos2D.x, y: pos2D.y, w: bounds, h: bounds },
          pos2D
        );
        Data.indexCounter++;
      }
  ...
  generateEmptySuggestions: function () {
    ...
    for (var i = 0; i < curatedSuggestionsList.length - 1; i++) {
      if (
        !~curatedSuggestionsList[i + 1].value.indexOf(
          curatedSuggestionsList[i].value
        )
      ) {
  ...
  playSound: function (soundIndex, time, velocity) {
    ...
    this.player.start(
      chunkIndex,
      time,
      soundIndex * this.soundLength + this.soundOffset,
      this.soundLength - this.soundReleaseTime,
      0,
      velocity
    );
  },
  ...
  suggest: function (value) {
    ...
    for (var key in Data.autoSuggest) {
      if (Data.autoSuggest.hasOwnProperty(key)) {
        ...
        if (
          value.toUpperCase() === key.substring(0, totalLetters).toUpperCase()
        ) {
          Data.suggestionList.push({
            value: key,
            tally: Data.autoSuggest[key],
          });
        }
      }
    }
  ...
  randomizeSoundIndexes: function () {
    ...
    if (this.filteredWord) {
      ...
      if (this.filteredList.length > 0) {
        ...
        var filteredListWithMeta = this.filteredList.map(
          function (item, index) {
