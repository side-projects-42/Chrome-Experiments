...
export class Sidebar extends EventEmitter {
  constructor() {
    ...
    this.fileDrop.on("dropped", () =>
      this.element.dispatchEvent(new CustomEvent("dropped"))
    );

    // render(this._html(), this.element)

    this.audioInputs = new AudioInput(
      this.element.querySelector("#audioInput"),
      videoFiles,
      this.element.querySelector("#mic"),
      this.fileDrop,
      this.element.querySelector("#inputSelect")
    );
    ...
    this.element.addEventListener("skiptocontent", () =>
      document.body.querySelector("#main-canvas").focus()
    );
    ...
    const remoteUrl =
      "https://storage.googleapis.com/gweb-access-music.appspot.com/";
  ...
  initialize() {
    ...
    this.element
      .querySelector("#piano-enable")
      .addEventListener(
        "change",
        ({ detail }) => (this.enablePolyphonic = detail.checked)
      );
    this.element
      .querySelector("acc-learn-more-group")
      .addEventListener("tutorial", () => this.fileDrop.clear());
  ...
  set grid(g) {
    if (this._options) {
      this._options.forEach(
        ({ viz, grid }) => (viz.showGrid = g && grid !== false)
      );
    }
  }
