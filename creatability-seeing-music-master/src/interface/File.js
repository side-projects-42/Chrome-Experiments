...
export class File extends EventEmitter {
  constructor() {
    ...
    this.audioMediaSource = Tone.context.createMediaElementSource(
      this.audioElement
    );
    if (FILE_PLAYS_SOUND) {
  ...
  initialize() {
    ...
    this.videoMediaSource = Tone.context.createMediaElementSource(
      this.videoElement
    );
    if (FILE_PLAYS_SOUND) {
  ...
  _readFile(file, url) {
    ...
    if (file) {
      ...
      const mediaElement = file.type.includes("audio")
        ? this.audioElement
        : this.videoElement;
