...
export class Pitch {
  constructor(source) {
    ...
    this._pichfinder = Pitchfinder.Macleod({
  ...
  _getMedian() {
    return this._smoothedPitches.slice().sort()[
      Math.floor(this._smoothedPitches.length / 2)
    ];
  }
  ...
  getPitch() {
    ...
    this._smoothedProb =
      this._smoothedProb * smoothing + probability * (1 - smoothing);
