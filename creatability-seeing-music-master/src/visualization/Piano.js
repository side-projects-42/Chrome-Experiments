...
export class Piano extends Visualization {
  constructor(source) {
    ...
    this._noteOffsets = [0, 0.5, 1, 1.5, 2, 3, 3.5, 4, 4.5, 5, 5.5, 6].map(
      (v) => v / 7
    );
  ...
  drawOctave(context, octave, notes) {
    ...
    const octaveLeft =
      scaleMidi(octaveStartNote) * this.height + this.noteWidth / 2;
      ...
      if (draw) {
        ...
        if (this.containsNote(notes, midiNote)) {
          ...
          const color = chroma
            .mix(
              "black",
              this.fullColor ? midi2Color(midiNote) : "white",
              opacity
            )
            .css();
