...
export class Model extends EventEmitter {
  ...
  initialize() {
    document
      .querySelector("acc-quiet-group")
      .addEventListener("pause", ({ detail }) => {
    ...
    if (!this._enabled && e) {
      ...
      if (!fallbackTester.hasWebGL()) {
        setTimeout(() => {
          this.emit("error", e);
          document
            .querySelector("acc-snackbar")
            .setAttribute(
              "message",
              "Cannot open WebGL context on this device."
            );
        }, 100);
        return;
      }
        ...
        try {
          ...
          if (WebMidi.supported) {
            document
              ...
              .setAttribute(
                "message",
                "Choose an audio file to transcribe, or play live with a MIDI keyboard."
              );
          } else {
            document
              .querySelector("acc-snackbar")
              .setAttribute("message", "Choose an audio file to transcribe.");
          }
        } catch (e) {
          this.loading = false;
          this.emit("error", e);
          console.log(e);
          document
            .querySelector("#error-snack")
            .setAttribute("message", "Transcription not supported");
        }
      });
    }
  ...
  async setFile(file, url) {
    if (this._enabled) {
      ...
      this.loader.setAttribute(
        "label",
        "Transcribing Audio (this might take a while)"
      );
